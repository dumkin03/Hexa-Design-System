...
description: >
  ...
  gate after every tier, and a hardened execute-and-verify protocol learned from real mistakes.
  Also covers a post-build efficiency pass — splitting an already-built Tokens (Tier 2) collection
  into a multi-mode "Themed" sub-collection and a single-mode "Static" sub-collection when only
  some functional groups actually vary across brand/theme modes — trigger on phrases like "only
  Primary needs all these modes", "split Tokens into themed and static", or "make the token
  collection more efficient."
compatibility:
  tools:
    - figma-console:figma_get_variables   # collection inspection, alias resolution (proposal phases)
...
color values. Phase 5 carries extra hardening, learned from a real near-loss of source data
during execution — read it carefully, it is not boilerplate.
...
## Standing naming & structure conventions

Unless the user says otherwise, every run of this skill should produce:

- **Collection names**: `1. Primitives`, `2. Tokens`, `3. Semantics` (numbered prefix keeps them in
  build order in Figma's collection picker, regardless of alphabetical sort).
- **Top-level grouping**: every variable in all three collections lives under a `Color/` group
  (e.g. `Color/Blue/550`, `Color/Primary/550`, `Color/Icon/Brand/default`) — never bare at the
  collection root. This is deliberate scaffolding: it leaves the collection root free for future
  sibling groups (`Typography/`, `Scale/`, `Spacing/`, etc.) to be added later without restructuring
  what already exists.
- **Variable order**: ascending, smallest value first (e.g. `50, 55, 60, 75, 100, 150 … 950`), not
  smallest-last. See Phase 5 — this must be set at creation time; it cannot be fixed afterward.
- **Tokens may split by mode-variance.** When the file has multiple brand/theme modes but only
  some functional Token groups actually resolve differently per mode (commonly only `Primary`),
  `2. Tokens` can split into `2a. Tokens - Themed` (multi-mode, holding only the groups that
  genuinely vary) and `2b. Tokens - Static` (single-mode, holding the groups that don't). This is
  an optional efficiency pass — see Phase 3.5 — not a default outcome of the initial build; only
  do it when asked, or when explicitly flagged during an audit and approved.
...
Phase 3 — Tier 2: Tokens proposal         (read-only)
    ...
    └── Multi-mode switching lives here, never at Tier 1. Function names only — no literal hue names.
         ↓
    ⛔ HUMAN GATE C — approve Token groups, sources, and full-ramp inclusion
         ↓
Phase 3.5 — Tokens mode-variance split        (optional, read-only until its own gate)
    └── Audit which Tier 2 functional groups actually resolve differently per mode vs. resolve
         identically across all modes
    └── If asymmetric, propose splitting into 2a. Tokens - Themed (multi-mode, variant groups
         only) and 2b. Tokens - Static (single-mode, invariant groups only)
         ↓
    ⛔ HUMAN GATE C.5 — confirm split scope (which groups move, which stay) before any writes
         ↓
...
holding an outdated iteration of the same plan). Specifically look for:
...
**This check must be repeated separately for any group excepted from renumbering** (e.g. a
Neutral ramp kept at `0–100` instead of `50–950`). "Excepted from renumbering" usually means
*keep the number range*, not necessarily *keep the original direction* — these are two separate
decisions. Confirm both explicitly: does the excepted group keep the original range AND the
original direction, or just the range? Getting this conflated cost a real rework cycle — the
group was originally built preserving the old file's actual direction, when the intent was for
it to follow the *same* light-to-dark-as-numbers-increase convention as every other group, just
on a 0–100 scale instead of 50–950.
...
**and target bounds**. Two separate things can fall short: not enough distinct values to fill
every step, *and* not enough range to reach the convention's boundary value (e.g. a group with a
genuine 18 distinct hex values may still only span `50–900` if that's all the real ramp covers,
falling short of a `950` boundary used elsewhere). Don't assume a convention's nominal endpoint is
actually reachable — verify the real source data's count and span before proposing numbers. If a
group has fewer real values than the step count requires, do **not** silently interpolate. Flag
it as a rule conflict against "never invent variables," and get explicit per-group sign-off
before generating any interpolated values. Always check whether more real values for that hue
exist elsewhere in the file before proposing interpolation. If a group has enough values to fill
all needed steps but stops short of the convention's outer boundary, that's a separate, smaller
question — best resolved later, after the initial build, by relabeling the topmost/bottommost
step rather than inventing a new color for it (see Phase 5 "Renaming vs. rebuilding").
...
- Apply the confirmed direction and step-numbering scheme to every group uniformly — but verify
  ...
  textbook scale. A "18-step Tailwind ramp" in the wild may genuinely run `50–900`, not `50–950`;
  don't write a build script around an assumed scheme without checking first.
...
## Phase 3.5 — Tokens mode-variance split (optional)

This phase only runs when asked, or when an audit surfaces the asymmetry and the user approves
acting on it. It doesn't change at the time of the original Phase 3 build — it's a retrofit
applied to an already-built `2. Tokens` collection (or run inline if the asymmetry is obvious at
Gate C time).

**Audit.** For every Tier 2 functional group, check whether its per-mode values actually differ
or are identical across every mode. In practice this usually means: one brand-driving group
(typically `Primary`) mode-switches across different source ramps, while the rest
(`Secondary`/`Tertiary`/`Error`/`Neutral`/`Success`, etc.) alias the same source in every mode —
i.e. they carry N identical mode-entries for no functional reason.

**Propose.** If the asymmetry is real, propose:
- `2a. Tokens - Themed` — same N modes as the original `2. Tokens`, holding only the
  genuinely-variant group(s).
- `2b. Tokens - Static` — a single mode, holding the invariant groups, each collapsed to one
  value (take any mode's value — by construction they're all identical; default to the
  collection's "Default" mode for clarity).
- Leave non-color content already living in `2. Tokens` (Typography, Border-Radius/Width, or
  anything else sharing the collection) **untouched** unless the user explicitly includes it in
  scope — don't assume "split Tokens" means split everything in it. Confirm scope explicitly if
  the collection holds more than the color groups under discussion.

### Human Gate C.5

Present the exact group-to-collection mapping (which groups go to Themed, which to Static, which
stay where they are) before writing anything.

### Execute — build-first variant (not the Phase 5 delete-first protocol)

This operation doesn't change any value — it only relocates existing variables into new
collections. Because nothing needs reinventing, this can safely run **build-before-delete**,
which is lower-risk than the standard Phase 5 protocol *and* avoids needing to hardcode a value
snapshot, since the new variables can copy live values straight from the old ones before the old
ones are removed:

1. **Build additively.** For each variant group: create the matching multi-mode group in
   `2a. Tokens - Themed`, copying the **exact existing per-mode alias** for every mode (match by
   mode *name*, not mode ID — the new collection's mode IDs are freshly assigned at creation).
   For each invariant group: create the matching single-mode group in `2b. Tokens - Static`,
   copying the value from the old collection's `Default` mode. The old `2. Tokens` group is left
   fully intact at this point — nothing has been removed yet.
2. **Record the full old-ID → new-ID map** as the script's return value. Don't rely on being able
   to re-derive it later from names alone — collisions are possible (e.g. `Color/Primary/600`
   exists in both old and new at this point).
3. **Rebind Tier 3.** Walk every Tier 3 (Semantic) variable across every mode; wherever its alias
   target is a key in the old-ID → new-ID map, replace it with the new ID. This is a value-free
   operation — Tier 3's own values don't change, only which variable they point at.
4. **Verify before deleting anything:**
   - Re-walk every collection in the file and confirm zero remaining aliases (anywhere, not just
     in Semantics) target any old ID — an orphaned reference left after the next step is
     unrecoverable without redoing this phase.
   - Spot-check resolved values end-to-end for at least one variant-sourced Semantic variable
     (confirm it still differs across modes as expected) and one invariant-sourced one (confirm
     it's now identical across modes, matching what it always resolved to).
5. **Delete the old variables** — isolated call, delete-only, no creation in the same script.
   Only the specific color variables that were relocated; never the whole `2. Tokens` collection
   if non-color content remains in it.
6. **Final verify.** Re-fetch the full variable summary. Assert: old group variables are gone,
   `2a`/`2b` counts match what was created, and anything left behind in `2. Tokens` is exactly
   what was confirmed in scope (e.g. Typography, Border-Radius/Width) — nothing color-related and
   nothing accidentally orphaned.
...
skill — a real attempt at this exact rebuild deleted a tier's only remaining source data and
crashed before rebuilding it, because delete and create were combined in one script. The
protocol below exists specifically because of that. Follow it literally, every time.
...
### Creation order is permanent — there is no reorder API

Figma's plugin API has no method to reorder variables within a collection after creation
(`collection.variableIds` is read-only, and no `reorderVariable`/`moveVariable` method exists as of
this writing — confirmed by direct introspection, not assumption). The order variables are
**created in** is the order they display in forever, until someone deletes and recreates them.
This means:
- Always create variables in **final intended visual order** (ascending: smallest value first,
  e.g. `50, 55, 60, 75, 100, 150 … 950`) in the very first build pass — never build descending and
  plan to "fix the order later."
- If you discover after building that the order is wrong, the only fix is a full delete-and-rebuild
  of every dependent tier (Primitives → Tokens → Semantic), because rebuilding Primitives generates
  new variable IDs that every Token and Semantic alias must be re-pointed to. Treat this the same
  as any other Phase 5 rebuild: isolated delete, isolated build, full re-verify.
- Before writing the build script, explicitly list out the exact creation-order sequence of steps
  and sanity-check it reads smallest-to-largest before running anything.
...
- **Watch for rename collisions.** If a relabeling scheme is a permutation (e.g. flipping a
  ...
  name" error, and a script that errors mid-loop may leave the set in a half-renamed state.
  Always do this as **two passes**: first move every affected variable to a unique temporary name
  (e.g. prefix `TEMP_`), then assign the final target names in a second pass. This guarantees no
  collision regardless of the permutation's structure.
...
- Stop immediately. Don't run further plugin scripts.
...
- Ask them to press Undo (`Cmd+Z` / `Ctrl+Z`) in Figma Desktop immediately, before anything else
  happens in the file. Plugin-driven mutations from a single script execution are typically
  grouped into one undo step, so a prompt single undo is often fully recoverable.
...
ran). Cheap to verify, expensive to assume.

### If the Desktop Bridge connection drops mid-task

A connection failure is not data loss — the file itself is unaffected. Ask the person to reopen
the plugin and confirm the connection, then re-verify current state before resuming. Don't assume
anything about what did or didn't complete before the drop.
...
| Tokens split (if Phase 3.5 ran) | Zero remaining aliases anywhere in the file targeting a pre-split Tier 2 ID; variant-sourced Semantic variables still resolve differently per mode, invariant-sourced ones resolve identically; non-color content left in `2. Tokens` is exactly what was scoped to stay |
...
**Building in descending order, planning to fix it later.** There is no reorder API — Figma
preserves whatever order variables were created in, permanently. Building `950 → 50` and assuming
the order can be flipped afterward forces a full delete-and-rebuild of every dependent tier. Always
create in final ascending order (`50 → 950`) on the first pass.

**Forgetting the standing naming/structure conventions.** Collections should be `1. Primitives`,
`2. Tokens`, `3. Semantics`, and every variable should sit under a `Color/` group — not bare at
the collection root. Easy to default back to plain `Color Primitives`-style names out of habit;
check against the "Standing naming & structure conventions" section before running Phase 5.

**Trusting "that's already fixed" without re-checking live state.** Earlier attempts can fail
silently partway through. Re-inspect before proceeding, every time — it's cheap insurance against
building on a false assumption.

**Assuming "split Tokens" means splitting everything the collection holds.** `2. Tokens` can carry
non-color content (Typography, Border-Radius/Width, anything else sharing the collection). A
request to split the color groups into Themed/Static doesn't imply moving those too — confirm
scope explicitly per group family before building, not just for the color groups under discussion.

**Trying to pass a large ID map into a script via a placeholder.** A script parameter can't
reference a value returned by an earlier tool call — `const map = __PREVIOUS_RESULT__` throws a
plain `ReferenceError`, it isn't resolved against prior output. Embed the old-ID → new-ID map (or
any other carried-forward data) as a literal JSON object directly in the next script's source.
